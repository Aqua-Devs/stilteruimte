...
export default function Home() {
  ...
  const features = [
    { icon: '📝', title: 'Privé Dagboek', desc: 'Schrijf wanneer je wilt. Geen druk, geen verwachtingen. Jouw gedachten blijven volledig privé en veilig bij jou.' },
    { icon: '💭', title: 'Zachte Reflectie', desc: 'Optionele dagelijkse prompts die je helpen verkennen wat je voelt. Nooit opdringerig, altijd optioneel.' },
    { icon: '🌊', title: 'Emotie Tracking', desc: 'Eenvoudig bijhouden hoe je je voelt. Visueel, zonder pressure. Zie patronen ontstaan in je eigen tijd.' },
    { icon: '🌱', title: 'Jouw Progressie', desc: 'Kijk terug op je reis wanneer je er klaar voor bent. Herken groei die je zelf misschien niet opmerkt.' }
  ]
  ...
  return (
    <main className="relative overflow-hidden">
      ...
      <nav className="relative z-50 px-[5%] py-8 flex justify-between items-center animate-[slideDown_0.8s_ease-out]">
        <div className="font-serif text-2xl font-light text-soft-black tracking-[2px]">STILTE RUIMTE</div>
        <ul className="hidden md:flex gap-10 items-center">
          ...
          <li><a href="#ai" className="text-warm-gray text-sm font-light hover:text-deep-sage transition-colors relative group">AI Begeleiding<span className="absolute bottom-[-5px] left-0 w-0 h-[1px] bg-sage transition-all duration-300 group-hover:w-full" /></a></li>
      ...
      <section className="relative z-10 min-h-[85vh] flex items-center px-[5%] py-16">
        <div className="max-w-[700px]">
          <h1 className="font-serif text-6xl md:text-7xl font-light text-soft-black leading-[1.1] mb-8 tracking-tight">Een veilige plek voor je gedachten na verlies</h1>
          <p className="text-xl text-warm-gray mb-12 font-light">Rouw heeft geen tijdslijn. Verwerkingstijd is persoonlijk. StilteRuimte begeleidt je zachtmoedig door je eigen reis, zonder druk of oordeel.</p>
          <div className="flex gap-4">
            <Link href="/auth/login" className="inline-block px-12 py-5 bg-white border-2 border-sage text-sage rounded-full text-base font-normal transition-all duration-400 hover:bg-sage hover:text-white hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(156,168,150,0.3)]">Inloggen</Link>
            <Link href="/auth/signup" className="inline-block px-12 py-5 bg-sage text-white rounded-full text-base font-normal transition-all duration-400 hover:bg-deep-sage hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(156,168,150,0.3)]">Maak account aan</Link>
          </div>
        </div>
      </section>

      <section id="functies" className="relative z-10 px-[5%] py-32 bg-gradient-to-b from-transparent via-mist/30 to-transparent">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 max-w-7xl mx-auto">
          {features.map((feature, i) => (
            <div key={i} className="scroll-reveal opacity-0 translate-y-12 transition-all duration-800 bg-white/60 backdrop-blur-md p-12 rounded-[30px] border border-sage/20 hover:-translate-y-2.5 hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] hover:border-sage">
              <div className="w-[60px] h-[60px] bg-gradient-to-br from-sage to-deep-sage rounded-full flex items-center justify-center mb-6 text-2xl">{feature.icon}</div>
              <h3 className="font-serif text-2xl font-normal mb-4 text-soft-black">{feature.title}</h3>
              <p className="text-base leading-relaxed text-warm-gray">{feature.desc}</p>
            </div>
          ))}
        </div>
      </section>

      <section id="ai" className="scroll-reveal opacity-0 translate-y-12 transition-all duration-800 relative z-10 px-[5%] py-32 bg-white">
        <div className="grid md:grid-cols-2 gap-20 max-w-7xl mx-auto items-center">
          <div className="relative h-[500px] bg-gradient-to-br from-mist to-cream rounded-[40px] flex items-center justify-center overflow-hidden">
            <div className="absolute w-[200%] h-[200%] opacity-10" style={{backgroundImage: 'radial-gradient(circle, var(--sage) 1px, transparent 1px)', backgroundSize: '30px 30px'}} />
            ...
            <ul className="space-y-0">
              {aiFeatures.map((item, i) => (
                <li key={i} className="py-5 border-b border-sage/20 text-base text-warm-gray flex items-start gap-4">
                  <span className="text-sage font-semibold text-lg flex-shrink-0">✓</span>{item}
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="scroll-reveal opacity-0 translate-y-12 transition-all duration-800 relative z-10 px-[5%] py-32 bg-mist">
        <div className="max-w-4xl mx-auto p-16 bg-white rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.05)]">
          <p className="font-serif text-3xl font-light leading-relaxed text-soft-black mb-8 italic">
            ...
            Na het verlies van mijn vader wist ik niet waar ik mijn gedachten moest laten. StilteRuimte gaf me een plek om alles op te schrijven zonder dat ik bang hoefde te zijn voor oordeel. De AI stelde precies de juiste vragen op momenten dat ik vast zat. Het voelde als iemand die echt luisterde.
          </p>
      ...
      <section className="scroll-reveal opacity-0 translate-y-12 transition-all duration-800 relative z-10 px-[5%] py-40 text-center">
        ...
        <p className="text-xl text-warm-gray mb-12 max-w-[600px] mx-auto">StilteRuimte is volledig gratis. Geen verborgen kosten, geen tijdslimiet. Begin vandaag met verwerkingstijd die bij jou past.</p>
      ...
      <footer className="relative z-10 px-[5%] py-16 bg-soft-black text-white/60 text-center">
        <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-8">
          <div className="font-serif text-2xl font-light tracking-[2px]">STILTE RUIMTE</div>
          ...
          <p>© 2026 StilteRuimte. Gemaakt met zorg.</p>
        </div>
      </footer>
      <RegisterSW />
    </main>
